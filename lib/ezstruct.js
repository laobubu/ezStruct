...
function createContext(source, context) {
  ...
  /** @type {{schema: Struct, expression: string}[]} */
  var compileQueue = []
  ...
  do {
    ...
    source = source.replace(/(\s*)(struct\s*[^{]*\s*){\s*([^{}]+)\s*}\s*(.)/g,
      function (full, lead, name, content, tail) {
        ...
        compileQueue.push({ expression: content, schema: ctx[name] })
  ...
  compileQueue.forEach(o => {
    var schema = o.schema
    schema.compile(o.expression)

    var enc = privated.encoding
    if (enc) {
      schema.struct.forEach(field => {
        if (/^char\[/.test(field.type)) field.encoding = enc
      })
    }
  })
...
createContext.setStringEncoding = function (enc) {
  privated.encoding = enc
}
